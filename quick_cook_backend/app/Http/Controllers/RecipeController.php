...
class RecipeController extends Controller
{
    ...
    public function match(Request $request)
    {
        ...
        if ($request->filled('difficulty')) {
            ...
            $allowedIds = Recipe::query()->where('difficulty', $difficulty)->pluck('id')->map(static fn ($id): int => (int) $id)->all();
            $allowedFlip = array_fill_keys($allowedIds, true);
            $rankedRows = array_values(array_filter($rankedRows, static function ($row) use ($allowedFlip) {
                return isset($allowedFlip[(int) $row->recipe_id]);
            }));
        ...
        if ($request->filled('max_cooking_time')) {
            $maxCooking = (int) $request->input('max_cooking_time');
            $allowedIds = Recipe::query()->where('cooking_time', '<=', $maxCooking)->pluck('id')->map(static fn ($id): int => (int) $id)->all();
            $allowedFlip = array_fill_keys($allowedIds, true);
            $rankedRows = array_values(array_filter($rankedRows, static function ($row) use ($allowedFlip) {
                return isset($allowedFlip[(int) $row->recipe_id]);
            }));
